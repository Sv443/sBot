...
module.exports.run = (client, message, args) => {
    ...
    fs.readdirSync("./commands/").forEach(file => { // get all available commands
        ...
        if(require("../commands/" + file).isAdminCommand == (null || undefined) && require("../commands/" + file).isDevCommand == (null || undefined)) {
            ...
            if(fArgs && !jsl.isArrayEmpty(fArgs))
                cargs = " [" + require("../commands/" + file).args.join("], [") + "]";

            if(require("../commands/" + file).category == "Bot Specific")
                botsp.push("`" + settings.command_prefix + file.replace(".js", cargs + "` - ") + require("../commands/" + file).help + "\n");
            else if(require("../commands/" + file).category == "Fun")
                fun.push("`" + settings.command_prefix + file.replace(".js", cargs + "` - ") + require("../commands/" + file).help + "\n");
            else if(require("../commands/" + file).category == "Knowledge")
                knowldg.push("`" + settings.command_prefix + file.replace(".js", cargs + "` - ") + require("../commands/" + file).help + "\n");
        }
    });
