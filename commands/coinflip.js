...
module.exports.run = (client, message, args) => {
    try {
        ...
        if(flipTms <= 1 || flipTms > settings.command_settings.coinflip.max)
            return message.reply("please enter a number between 2 and " + settings.command_settings.coinflip.max + " (example: `" + settings.command_prefix + "coinflip 5`)");

        if(isNaN(flipTms) || jsl.isEmpty(flipTms))
            return flipCoin(message);
        else {
